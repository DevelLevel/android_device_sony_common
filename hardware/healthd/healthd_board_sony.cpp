...
struct soc_led_color_mapping s_soc_leds[3] = {
    {15, RED_LED},
...
static int set_tricolor_led(int on, int color)
{
    ...
    for (i = 0; i < (int)ARRAY_SIZE(s_leds); i++) {
        if ((color & s_leds[i].color) && (access(s_leds[i].path, R_OK | W_OK) == 0)) {
            fd = open(s_leds[i].path, O_RDWR);
            if (fd < 0) {
...
static int set_battery_soc_leds(int soc)
{
    ...
    for (i = 0; i < (int)ARRAY_SIZE(s_soc_leds); i++) {
        if (soc <= s_soc_leds[i].soc)
            break;
    }
    color = s_soc_leds[i].color;
    if (old_color != color) {
        set_tricolor_led(0, old_color);
        set_tricolor_led(1, color);
        old_color = color;
        LOGV("soc = %d, set led color 0x%x\n", soc, s_soc_leds[i].color);
    }
    return 0;
}
